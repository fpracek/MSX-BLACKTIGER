...
slots = [s for s in range(224) if s not in RESERVED_SLOTS]
...
lines.append(f"#define LEVEL_TILES {len(order)}")
...
flat_cells = sum(1 for r in range(ROWS) for c in range(COLS)
                 if flat[slot_of[uniq[cells[r][c]]]] != 0xFF)
print(f"Tile uniformi: {n_flat}, celle mappa piatte: {flat_cells}/{ROWS*COLS}")
lines.append("")
lines.append("const unsigned char g_TileFlat[224] = { "
