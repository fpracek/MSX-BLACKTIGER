...
for t in tiles:
    v = int(t[0, 0])
    flat_tbl.append(((v << 4) | v) if np.all(t == v) else 0xFF)
lb = bitmap(cell_solid) + bitmap(cell_climb) + flat_tbl
(PROJ / "bin" / "level1.bin").write_bytes(lb)
print(f"level1.bin: {len(lb)}B (solid+climb bitmap + flat table)")
...
lines.append("// g_TileFlat: in coda a level1.bin (RAM), caricata da LoadLevelData")
